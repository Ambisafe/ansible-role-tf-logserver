locals {
  es_arn      = "${element(concat(aws_elasticsearch_domain.es_vpc.*.arn,list("")),0)}"
  es_endpoint = "${element(concat(aws_elasticsearch_domain.es.*.endpoint,list("")),0)}"
  es_domain_id= "${element(concat(aws_elasticsearch_domain.es.*.domain_id,list("")),0)}"
}
